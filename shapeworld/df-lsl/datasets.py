...
def get_max_hint_length(data_dir=None):
    ...
    for split in ['train', 'val', 'test', 'val_same', 'test_same']:
        for tf in ['hints.json', 'test_hints.json']:
            hints_file = os.path.join(data_dir, split, tf)
...
class ShapeWorld(data.Dataset):
    ...
    def _add_noise(self,
        ...
        if self.noise_type == 'gaussian':
            support_instance_noise = torch.zeros(*support.shape)
            query_instance_noise = torch.zeros(*query.shape)
        else:

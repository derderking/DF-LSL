...
if __name__ == "__main__":
    ...
    optimizer = optfunc(params_to_optimize, lr=args.lr, weight_decay=1e-5)
    ...
    def train(epoch, n_steps=100):
        ...
        for batch_idx in range(n_steps):
            ...
            for num in range(examples_rep.size(1)):
                ...
                pred_loss += cal_pred_loss(hint_model,hint_seq,hint_length,image_rep,label,multimodal_model, scorer_model, one_ex, args.infer_hyp, args.multimodal_concept, args.poe)
            # Prediction loss
            # Hypothesis loss
            if args.use_hyp:
                for num in range(examples_rep.size(1)):
                    one_ex = examples_rep[:,num,:]
                    hypo_loss += cal_hypo_loss(train_vocab_size, batch_size, n_ex, hint_seq,hint_length, examples_rep, hint_model, multimodal_model, 
                        proposal_model,args.use_hyp,args.predict_image_hyp,args.predict_hyp,args.predict_hyp_task,args.multimodal_concept,one_ex)
            #    for num in range(examples_rep.size(1)):
            #        one_ex = examples_rep[:,num,:]
            #        hypo_loss += cal_hypo_loss(train_vocab_size, batch_size, n_ex, hint_seq,hint_length, examples_rep, hint_model, multimodal_model, 
            #            proposal_model,args.use_hyp,args.predict_image_hyp,args.predict_hyp,args.predict_hyp_task,args.multimodal_concept,one_ex)
                ...
                loss = pred_loss
            loss = loss / float(examples_rep.size(1))
            loss_total += loss.item()

            optimizer.zero_grad()
            loss.backward()
            torch.nn.utils.clip_grad_norm_(params_to_optimize, 2)
            optimizer.step()

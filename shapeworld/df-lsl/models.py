...
class ModelScorer(DotPScorer):
    def __init__(self, hidden_size, dropout=0.0, identity_debug=False):
        super(ModelScorer, self).__init__()
        self.bilinear = nn.Linear(hidden_size, hidden_size, bias=False)
        ...
        if self.dropout_p > 0.0:
            ...
            self.dropout = lambda x: x
        self.bilinear.weight = nn.Parameter(torch.eye(hidden_size, dtype=torch.float32))
        self.bilinear.weight.requires_grad = False
        self.fc2.weight = nn.Parameter(torch.eye(hidden_size, dtype=torch.float32))
    ...
    def score(self, x, y):
        wy = self.bilinear(y)
        wy = self.dropout(wy)
        wx = self.dropout(self.fc2(x))
        return super(ModelScorer, self).score(wx, wy)
    ...
    def batchwise_score(self, x, y):
        ...
        wy = self.bilinear(y.view(batch_size * n_examples,
                                  -1)).unsqueeze(1).view_as(y)
        wy = self.dropout(wy)
        wx = self.fc2(x.view(batch_size * n_examples,
                                  -1)).unsqueeze(1).view_as(x)
        wx = self.dropout(wx)
        # wy: (batch_size, n_examples, h)
        return super(ModelScorer, self).batchwise_score(wx, wy)

...
class ProtoNet(nn.Module):
    ...
    def correct(self, x, return_loss=False):
        ...
        """
        res_list = self.set_forward(x)
        scores = torch.stack(res_list,dim = 0).mean(0)
    ...
    def train_loop(self, epoch, train_loader, optimizer, args):
        ...
        for i, (x, target, (lang, lang_length, lang_mask)) in enumerate(train_loader):
            ...
            # ==== CLASSIFICATION LOSS ===-
            cls_loss_1, z_support, z_query = self.set_forward_loss_l3(
                x, (lang, lang_length), return_z=True
            )
            cls_loss_2, z_support, z_query = self.set_forward_loss(x, return_z=True)
    ...
    def set_forward(self, x, is_feature=False, return_z=False):
        ...
        z_support, z_query = z_support_orig, z_query_orig

        z_query = z_query.contiguous().view(self.n_way * self.n_query, -1)
        z_support = z_support.contiguous()
        #z_proto = z_support.mean(1)

        for num in range(z_support.size(1)):
            scores = self.score(z_support[:,num,:], z_query, support_type="img", query_type="img")
            res_list.append(scores)
        if return_z:
            return res_list, z_support_orig, z_query_orig
        return res_list
    ...
    def set_forward_loss(self, x, return_z=False):
        ...
        if return_z:
            res_list, z_support, z_query = self.set_forward(x, return_z=True,)
        else:
            res_list = self.set_forward(x)
        loss = 0
        for scores in res_list:
            loss += self.xent_loss(scores, y_query) / len(res_list)
        if return_z:
    ...
    def set_forward_loss_l3(self, x, lang_all, return_z=False):
        ...
        z_support_img = z_support.contiguous().view(self.n_way * self.n_support, -1)
        ...
        loss = 0
        for num in range(z_support_lang.size(1)):
            # lang <-> img comparison
            scores = self.score(
                z_support_lang[:,num,:], z_query_img, support_type="lang", query_type="img"
            )
            scores = F.log_softmax(scores, 1)
            loss += self.nll_loss(scores, y_query) / z_support_lang.size(1)
            scores = self.score(
                z_support_lang[:,num,:], z_support_img, support_type="lang", query_type="img"
            )
            scores = F.log_softmax(scores, 1)
            loss += self.nll_loss(scores, y_support) / z_support_lang.size(1)
        if return_z:
            return loss, z_support, z_query
        return loss
